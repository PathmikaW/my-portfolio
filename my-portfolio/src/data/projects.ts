...
export const industryProjects: IndustryProject[] = [
  {
    ...
    highlights: [
      ...
      "Built the app's application state store (React Context + useReducer) covering session, multi-line/connection switching, loyalty tier, and force-update state.",
      'Enforced certificate/public-key pinning on every network request.',
      ...
      'Built a dual push-notification pipeline selecting between Firebase Cloud Messaging and Huawei HMS Push Kit at runtime to handle Android delivery across GMS and HMS devices.',
  ...
  },
  {
    ...
    highlights: [
      'Developed a fully custom, cross-platform native alarm engine from the ground up - ~2,700 lines of native Java (Android) and Swift/Objective-C (iOS) across 26 files, bridged to a unified JavaScript API, replacing reliance on third-party alarm libraries.',
      'Engineered Android alarm reliability using exact-alarm scheduling, foreground services, and wake-lock management to guarantee precise delivery despite Doze mode, App Standby, and OEM battery-optimization restrictions.',
      'Designed a reboot-persistence system that automatically re-arms all active alarms after device restart.',
      "Built the iOS alarm subsystem on Apple's local-notification scheduling framework, with custom gradual volume-ramp audio playback.",
  ...
  },
  {
    ...
    context: 'Telecom SMS-firewall AI/ML detection suite and business-intelligence platform, deployed as a centralized on-prem system.',
    ...
    highlights: [
      ...
      'OTP-bypass / grey-route module: combines Caesar-cipher/base64 CDR decryption, SentenceTransformer embeddings, and cosine-similarity matching against a rolling reference table to flag OTP-bypass and grey-route A2P traffic; the encoded-Unicode-pattern module layers in the same semantic-embedding approach.',
      ...
      'GenAI subsystem: a local LLM (Google Gemma via Ollama, orchestrated with LangChain-Ollama) supporting content review and translation suite-wide, backed by multilingual BAAI/bge-m3 embeddings (SentenceTransformers) for semantic similarity search, plus a FastText-based pipeline for language identification.',
      ...
      'Architected and led development of FirewallBI as a centralized on-prem deployment, evolving it from an early ELK-based analytics prototype into a productionized, containerized platform.',
      ...
      'Built hundreds of Logstash JDBC pipeline configurations, each executing hand-written PostgreSQL rollup queries, plus a parallelized Python reconciliation framework validating data integrity with Slack/email alerting.',
      'Contributed Random Forest risk scoring and social-graph-based SIM-box-ring detection to an R&D flash-call fraud-detection system built on CAMEL/CAP signaling with an inline IVR decision path.',
    ],
  },
  {
    id: 'sor',
    ...
    highlights: [
      ...
      'Led development of a centralized web-based reporting platform using asynchronous task processing and message queues (Celery) with containerized deployment (Docker).',
  ...
  },
  {
    ...
    highlights: [
      'Led the project team building HOPP and took it to production; owned the frontend architecture and core infrastructure (confirmed via authorship headers on the source).',
      'Built a custom backend-driven image CAPTCHA component instead of relying on a third-party reCAPTCHA package.',
      'Integrated the FriMi payment gateway (Nations Trust Bank) end-to-end - authentication, request/response handling, and error mapping - alongside multiple card-network payment methods.',
  ...
  {
    id: 'banglalink-smsc',
    name: 'SMSC Reporting Engineering (Bangladesh)',
    category: 'Data Platforms & AI/ML',
    ...
    highlights: [
      'Reverse-engineered and documented the CDR-to-report field mapping for the SMSC billing/reporting platform, writing AWK-based cross-validation scripts achieving ~99.9% accuracy against raw CDR logs.',
      ...
      'Diagnosed a ~100x query-performance regression caused by 7 missing indexes; added them without foreign-key constraints to avoid write-lock contention, substantially cutting query time.',
  ...
  {
    id: 'bangla-sms',
    name: 'SMS Opt-In/Opt-Out Platform (Bangladesh)',
    category: 'Web',
    context: 'SMS language-preference (opt-in/opt-out) platform for a telecom operator in Bangladesh.',
    ...
    highlights: [
      ...
      'Reviewed platform scope covering 5 user-role types, bulk MSISDN/language-preference management, a daily D-1 dump sync, and real-time push/pull REST APIs (low-latency QoS).',
  ...
  },
  {
    ...
    highlights: [
      'Authored technical architecture proposals and effort estimates for telecom operators across South Asia, Africa, and the Middle East, spanning AI/ML fraud and CVM platforms, large-scale CDR search infrastructure, and roaming/steering solutions.',
